...
2. If more than one P1 agent produced packets, run `ep-p1-data-curator` to consolidate the P1 set before P2. Skip this for a single P1 agent.
3. Assign P2 only after a single P1 packet or curated P1 dataset exists.
4. If more than one P2 agent produced packets, run `ep-p2-possibility-integrator` to consolidate the P2 set before P3. Skip this for a single P2 agent.
5. Assign P3 with P1 and P2 packets; require exactly one outcome: return to P1, return to P2, or advance to P4.
6. If more than one P3 agent judged the same substrate, run `ep-p3-dialectician` to consolidate the P3 judgments before P4. Skip this for a single P3 agent.
7. Assign P4 only with manifest, accumulated record, and P3 handoff.
8. If more than one P4 agent produced evaluations, run `ep-p4-ethics-sage` to decide the best authorized decision before controller finalization. Skip this for a single P4 agent.
...
3. When a scope has multiple P1 packets, route them through `ep-p1-data-curator` before P2.
...
6. When a scope has multiple P2 packets, route them through `ep-p2-possibility-integrator` before P3.
7. Run P3 passes to judge sufficiency, reconcile contradictions, select paths, and route recursion.
8. When a scope has multiple P3 packets over the same possibilities, route them through `ep-p3-dialectician` before P4.
9. Run P4 decisions only for scopes with sufficient P3 handoffs.
10. When a scope has multiple P4 evaluations, route them through `ep-p4-ethics-sage` before controller finalization.
